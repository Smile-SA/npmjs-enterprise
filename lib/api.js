...
api.initChangesSync = function() {
  ...
  if (!fs.existsSync(sequenceFile)) {
    request(config.skimdbURL, function(error, response, body) {
      if (!error && response.statusCode == 200) {
        var updateSequence = JSON.parse(body).update_seq;
        logger.debug('Creating %s with current update_seq on %s: %s', sequenceFile, config.skimdbURL, updateSequence);

        fs.writeFile(sequenceFile, updateSequence, function() {
          changeSync();
        });
      } else {
        throw error;
    ...
    var updateSequence = fs.readFileSync(sequenceFile);
    logger.debug('Restarting sync from updateSequence %s', updateSequence);
    changeSync();
  }
};
