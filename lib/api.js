...
api.initChangesSync = function() {
  ...
  var syncFromNow = !fs.existsSync(sequenceFile);
  if (syncFromNow) {
    logger.debug('Starting sync from latest sequence number on %s', config.skimdbURL);
  } else {
    logger.debug('Restarting sync from %s since sequence number %s', config.skimdbURL, parseInt(fs.readFileSync(sequenceFile, 'utf-8')));
  }
  ...
  changes(function(data, done) {
    ...
    fs.exists(filePath, function(exists) {
      if (exists) {
        ...
        fs.writeFile(filePath, JSON.stringify(data.doc), done);
      } else {
